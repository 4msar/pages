...
const config = {
    ...
    proxyDomains: ["cors.msar.workers.dev", "cors.msar.me"],
...
async function handleRequest(request) {
    ...
    */
        ...
        if (isProxyHost) {
          ...
          */
                    ...
                    if (ref.startsWith(`https://${url.host}/`)) {
                        let refInner = ref.substring(
                            `https://${url.host}/`.length,
                        );
                        if (refInner.startsWith(config.separator))
                            refInner = refInner.substring(
                                config.separator.length,
                            );
                        if (
                            refInner.startsWith("http://") ||
                            refInner.startsWith("https://")
                        ) {
                            try {
                    ...
                    if (resolved) {
                        ...
                        if (rebuilt) {
                            ...
                            url.searchParams.forEach((value, key) =>
                                ddgURL.searchParams.append(key, value),
                            );
                            targetURL = ddgURL;
                        } else {
                            ...
                            try {
                                ...
                                return new Response("Invalid URL request", {
                                    status: 400,
                                });
                            }
                        }
    ...
    const isWikipediaSite =
        config.specialSites.wikipedia.enabled &&
    ...
    newHeaders.set(
        ...
        config.browserEmulation.upgradeInsecureRequests,
    );
    ...
    try {
        ...
        if (
            ...
            let rewriter = new HTMLRewriter()
                .on(
                    "a[href]",
                    new LinkRewriter(targetURL, "href", currentProxyDomain),
                )
                .on(
                    "form[action]",
                    new LinkRewriter(targetURL, "action", currentProxyDomain),
                )
                .on(
                    "img[src]",
                    new LinkRewriter(targetURL, "src", currentProxyDomain),
                )
                .on(
                    "img[srcset]",
                    new SrcsetRewriter(targetURL, currentProxyDomain),
                )
                .on(
                    "source[srcset]",
                    new SrcsetRewriter(targetURL, currentProxyDomain),
                ...
                .on(
                    "object[data]",
                    new LinkRewriter(targetURL, "data", currentProxyDomain),
                )
                .on(
                    "track[src]",
                    new LinkRewriter(targetURL, "src", currentProxyDomain),
                ...
                .on(
                    "meta[content]",
                    new MetaContentRewriter(targetURL, currentProxyDomain),
                );

            // Handle base tag
            rewriter = rewriter.on(
                "base[href]",
                new BaseTagRewriter(targetURL, currentProxyDomain),
            );

            // Handle inline styles and attributes with URLs
            rewriter = rewriter.on(
                "*[style]",
                new StyleAttributeRewriter(targetURL, currentProxyDomain),
            );
            ...
            if (isWikipediaSite) {
                ...
                rewriter = rewriter.on(
                    "img[data-src]",
                    new LinkRewriter(targetURL, "data-src", currentProxyDomain),
                );
                // Handle Wikipedia's specific style elements
                rewriter = rewriter.on(
                    "style",
                    new StyleElementRewriter(targetURL, currentProxyDomain),
                );
            }
            ...
            if (config.fallback.enabled && config.fallback.autoReload) {
                rewriter = rewriter.on(
                    "head",
                    new HeadRewriter(targetURL.href),
                );
            }
        ...
        // Handle CSS content separately to rewrite URLs
            ...
            contentType.includes("application/x-stylesheet")
        ) {
            const currentProxyDomain = url.host;
        ...
        // Handle JavaScript to rewrite URLs directly embedded in code
            ...
            const rewrittenJS = rewriteJavaScript(
                jsText,
                targetURL,
                currentProxyDomain,
            );
        ...
        try {
            ...
            if (this.attributeName === "src" && element.tagName === "img") {
                ...
                element.setAttribute(
                    "onerror",
                    `this.onerror=null;if(this.src!==this.dataset.originalSrc){this.src=this.dataset.originalSrc;}`,
                );
            }
...
class StyleAttributeRewriter {
    ...
    element(element) {
        ...
        const rewrittenStyle = rewriteCSS(
            style,
            this.baseURL,
            this.proxyDomain,
        );
        ...
        function (match, urlMatch, directMatch) {
            ...
            try {
                ...
                return match.replace(
                    importUrl,
                    `https://${proxyDomain}/${config.separator}${absoluteURL.href}`,
                );
            } catch (e) {
                return match;
            }
        ...
        function (match) {
            return match.replace(
                ...
                function (urlMatch, quote1, url, quote2) {
                    ...
                    if (url.startsWith(`https://${proxyDomain}/`))
                        return urlMatch;
...
<body>
  <div class="container">
    ...
    <p class="example">
    Examples: 
    <a href="/------https://www.tsukuba.ac.jp/">tsukuba.ac.jp</a>, 
    <a href="/------https://www.jlu.edu.cn/">jlu.edu.cn</a>, 
    <a href="/------https://news.ycombinator.com">ycombinator.com</a>, 
    <a href="/------https://www.example.com">example.com</a>, 
    <a href="https://github.com/4msar/cors">Github Repo</a>
</p>
